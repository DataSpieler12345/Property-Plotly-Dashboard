...
def avg_price_map_figure(selected_region,selected_year):
    # load the csv 
...
def avg_price_bar_figure(selected_region, selected_year):
    # load data from csv 
    ...
    fig.update_layout(
        ...
        plot_bgcolor='#343a40',
        font_color='white',
        legend=dict(title=dict(text='Legend Title'), orientation='h', yanchor='bottom', y = 1.02, xanchor='right', x=1),
    )
...
def delta_box_plot_figure(selected_region, selected_year):
    ...
    fig.update_layout(
        ...
        plot_bgcolor='#343a40',
        font_color='white',
        legend=dict(title=dict(text='legend Title'), orientation='h', yanchor='bottom', y = 1.02),
    )
...
def delta_map_figure(selected_region, selected_year):
    # Load the CSV file 
    csv_file_path = f'processed_data/avg_price_delta/avg_prices_delta_{selected_year}.csv'
    data = pd.read_csv(csv_file_path)
...
def fastest_growing_map_figure(selected_year, selected_region,slider_value):
...
def fastest_declining_map_figure(selected_year, selected_region,slider_value):
    # Load the CSV file with price change data based on the selected year
    csv_file_path = f'processed_data/avg_price_delta/avg_price_delta_{selected_year}.csv'
    data = pd.read_csv(csv_file_path)
